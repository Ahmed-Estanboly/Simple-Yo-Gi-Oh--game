...
function endGame(loser, reason) {
    ...
    const reasonMessage = reason === "surrender"
...
function summonMonster(player, cardId) {
    ...
    renderHand(player);
    renderField(player);
    const summonedCard = document.querySelector(`#${player}-area .card[data-id='${card.id}']`);
    if (summonedCard) {
...
function drawPhase() {
    ...
    clickable(gameState.currentPlayer === "player"?"opponent":"player",false,false,false);
...
function battlePhase() {
    ...
    clickable(gameState.currentPlayer === "player"?"opponent":"player",true,false,false);
        ...
        cardElement.addEventListener('click', () => {
            ...
            for (let j = 0; j < gameState.players[opponent].field.length; j++) {
                ...
                opponentCardElement.addEventListener('click', () => {
                    ...
                    console.log(`[Battle] ${card.name} (${card.atk}) attacks ${opponentCard.name} (${opponentCard.atk}); difference=${damage}`);
...
async function startGame(){
    if (!startButton || !drawButton || !mainPhaseButton || !battlePhaseButton || !endTurnButton || !surrenderButton) {
